import React from 'react';

/* 
<svg
	aria-hidden="true"
	focusable="false"
	data-prefix="fas"
	data-icon="heart"
	class="svg-inline--fa fa-heart fa-w-16 fa-fw has-text-danger"
	role="img"
	xmlns="http://www.w3.org/2000/svg"
	viewBox="0 0 512 512"
>
	<path
		fill="currentColor"
		d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z"
	></path>
</svg>; */

export default class extends React.Component {
  render() {
    return (
			<>
				<div
					className="flex flex-col items-center text-center justify-center sm:p-16 p-6 text-gray-700"
					style={{ background: "#101010" }}
				>
					<p className="leading-5 tracking-tight">
						Made with
						<svg
							aria-hidden="true"
							focusable="false"
							data-prefix="fas"
							data-icon="heart"
							height="16"
							width="16"
							className="fill-current text-red-500 inline-block mx-1"
							role="img"
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 512 512"
						>
							<path
								fill="currentColor"
								d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z"
							></path>
						</svg>
						<a
							className="text-yellow-500 hover:text-yellow-300"
							href="https://nextjs.org/"
							target="_blank"
						>
							NextJS
						</a>{" "}
						&amp; Headless CMS service provided by{" "}
						<a
							className="text-yellow-500 hover:text-yellow-300"
							href="https://buttercms.com/"
							target="_blank"
						>
							ButterCMS
						</a>
					</p>
					<img src="https://cdn.buttercms.com/RyJ7UhcVTCRqrCFXwgCo" className="w-48" />
					<p className="text-sm">
						Copyright &copy; {new Date().getFullYear()} Efren Palacios {" "}
						<a
							className="text-yellow-500 hover:text-yellow-300"
							href="mailto:contact@efrenpalacios.dev"
							target="_blank"
						>
							contact@efrenpalacios.dev
						</a>
					</p>
				</div>
				{/* <Footer id="footer" style={{ background: "#101010" }}>
					<Container hasTextAlign="centered">
						<Content>
							<Columns>
								<Column>
									<div>
										Made with
										<FontAwesomeIcon
											className="has-text-danger"
											fixedWidth
											icon={faHeart}
										/>
										<a
											className="has-text-warning"
											href="https://nextjs.org/"
											target="_blank"
										>
											NextJS
										</a>{" "}
										& Headless CMS service provided by{" "}
										<a
											className="has-text-warning"
											href=" https://buttercms.com/"
											target="_blank"
										>
											ButterCMS
										</a>{" "}
										<Image
											style={{ width: "150px", margin: "0 auto" }}
											src="https://cdn.buttercms.com/RyJ7UhcVTCRqrCFXwgCo"
										/>
									</div>
								</Column>
							</Columns>
							<Content isSize="small">
								<div>
									Copyright &copy; {new Date().getFullYear()} Efren Palacios{" "}
									<a
										className="has-text-warning"
										href="mailto:contact@efrenpalacios.dev"
									>
										contact@efrenpalacios.dev
									</a>
								</div>
							</Content>
						</Content>
					</Container>
				</Footer> */}
			</>
		);
  }
} 